...
%
% Synthetic data is generated by integrating an ODE with a chosen force law.
% The ODE produces x_true and v_true. The analytical ground truth force is
% then computed directly from those clean trajectories:
%   F_analytical(n) = m*(v_{n+1}-v_{n-1})/(2*dt) + c*v_n + k*x_n
% This is the exact force the system experienced -- no model assumed.
%
% DDI recovers F using only noisy x_obs. The force panel compares:
%   Analytical: exact F from clean x_true, v_true (perfect data limit)
%   DDI:        F recovered from noisy x_obs alone
...
w_obs_v =  0.5;   % trust smoothed v estimate weakly
wx = 1.0;
wv = 1.0;
wF = 3.0;
wn = ones(N_ext, 1);
...
% Analytical ground truth force: exact residual from clean trajectories.
% F = m*a + c*v + k*x evaluated on true (noiseless) x and v.
% This is what DDI is trying to recover from noisy data alone.
F_analytical = zeros(Nexp, N_out);
for a = 1:Nexp
    for n = 2:N_out-1
        F_analytical(a,n) = m*(v_true_ext(a,n+1)-v_true_ext(a,n-1))/(2*dt) ...
                          + c*v_true_ext(a,n) + k*x_true_ext(a,n);
    end
    F_analytical(a,1)     = F_analytical(a,2);
    F_analytical(a,N_out) = F_analytical(a,N_out-1);
end
...
for a = 1:Nexp
    ...
    % Force: analytical ground truth (exact, from clean data) vs DDI
    % (recovered from noisy x_obs only). Gap between them is the cost
    % of having only noisy displacement measurements.
    subplot(Nexp, 3, (a-1)*3 + 3);
    plot(t_out, F_analytical(a,:), 'k-', 'LineWidth',1.5); hold on;
    plot(t_out, F_traj_out(a,:),   '--', 'Color',cRed, 'LineWidth',1.5);
    xlabel('t'); ylabel('F'); grid on; box on;
    title(sprintf('Exp %d - Force',a));
    if a==1, legend('Analytical (clean data)','DDI','Location','best'); end
end
