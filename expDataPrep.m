...
% nondimensional strain rate not absolute
strainRate = (-2 .* Rdotmatrix) ./ Rmatrix;      % same size as Rmatrix
strainRate(~isfinite(strainRate)) = NaN;

% --- Equilibrium-referenced log strain ---
strain = zeros(size(R_d));        
for j = 1:size(R_d,2)
    denom = max(Req_each(j), 1e-12);  % guard tiny tail estimates
